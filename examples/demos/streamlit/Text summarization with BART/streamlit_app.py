...
def create_pipeline(model, max_out_length):
    ...
    bart = BartTransformer.pretrained(model) \
        .setTask("summarize:") \
        .setMaxOutputLength(max_out_length) \
...
def fit_data(pipeline, data):
    ...
    #model = pipeline.fit(empty_df).transform(df)
    #return model
...
model_list = ["distilbart_xsum_12_6"]
...
max_out_length = st.slider("Select summerized length", min_value=100, max_value=(len(selected_text)-100), value=200, step=10)
