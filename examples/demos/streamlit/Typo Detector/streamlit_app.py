...
def create_pipeline():
    ...
    lemmatizer = Lemmatizer() \
        .setInputCols(["token"]) \
        .setOutputCol("lemma") \
        .setDictionary("/content/Text Preprocessing/AntBNC_lemmas_ver_001.txt", value_delimiter ="\t", key_delimiter = "->")
...
display_text = {
  "sentences": "Detected sentences in text:",
  "token": "Basic Tokens:",
  "stem": "Stemmed Tokens:",
  "lemma": "Lemmatized Tokens:",
  "normalized": "Tokens after removing punctuations:",
  "removed_stopwords": "Tokens after removing Stop Words:"
}

mappings = ["sentences", "token", "stem", "lemma", "normalized", "removed_stopwords"]
true_indices = [index for index, value in enumerate(selected_models) if value]

for val in true_indices:
  st.write(display_text[mappings[val]])
  st.dataframe(pd.DataFrame(output[mappings[val]], columns=[mappings[val]]))
