---
layout: model
title: Detect Persons, Locations, Organizations, Dates, Time, Numbers, and Destination Entities - UR (urduvec_140M_300d)
...
\``PERSON``, ``LOCATION``, ``ORGANIZATION``, ``DATE ``, ``DESIGNATION``, ``TIME``, ``NUMBER``
