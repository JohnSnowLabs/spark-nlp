...
name: snomed_sentence_resolver
date: 2021-11-02
tags: [es, snomed, entity_resolution, clinical, licensed]
...
[Download](https://s3.amazonaws.com/auxdata.johnsnowlabs.com/clinical/models/snomed_sentence_resolver_es_3.3.0_3.0_1635879359487.zip){:.button.button-orange.button-orange-trans.arr.button-icon}

## How to use

From NER entities (use any MedicalNer Model from our ModelsHub that detects, for example, diagnosis), then a NerConverter() (in case your model has B-I-O notation). Create documents using Chunk2Doc. Then Tokenize the chunk, use the `roberta_base_biomedical_es` Roberta Embeddings model, and then average with SentenceEmbeddings, as in the example.
...
{:.table-model}
|---|---|
|Model Name:|snomed_sentence_resolver|
