...
date: 2021-07-21
tags: [pt, br, legal, lener, ner, licensed]
...
[Download](https://s3.amazonaws.com/auxdata.johnsnowlabs.com/clinical/models/lener_bert_base_pt_3.1.2_3.0_1626895498932.zip){:.button.button-orange.button-orange-trans.arr.button-icon}
...
ner_model = NerDLModel.pretrained('lener_bert_base', 'pt') \
...
val ner_model = NerDLModel.pretrained('lener_bert_base', 'pt') 
    .setInputCols(['document', 'token', 'embeddings']) 
    .setOutputCol('ner')
...
ner_df = nlu.load('pt.ner.lener_bert_base').predict(text, output_level='token')
```
</div>
