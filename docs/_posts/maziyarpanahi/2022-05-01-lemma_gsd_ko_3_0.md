...
Pretrained Lemmatizer model (`lemma_gsd`) trained on Universal Dependencies 2.9 (UD_Korean-GSD) in Korean language.
...
[Copy S3 URI](s3://auxdata.johnsnowlabs.com/public/models/lemma_gsd_ko_3.4.3_3.0_1651416614863.zip){:.button.button-orange.button-orange-trans.button-icon.button-copy-s3}
...
```python
import nlu
nlu.load("ko.lemma").predict("""I love Spark NLP""")
```
