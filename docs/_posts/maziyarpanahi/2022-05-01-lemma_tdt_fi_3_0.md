...
Pretrained Lemmatizer model (`lemma_tdt`) trained on Universal Dependencies 2.9 (UD_Finnish-TDT) in Finnish language.
...
[Copy S3 URI](s3://auxdata.johnsnowlabs.com/public/models/lemma_tdt_fi_3.4.3_3.0_1651416591751.zip){:.button.button-orange.button-orange-trans.button-icon.button-copy-s3}
...
```python
import nlu
nlu.load("fi.lemma").predict("""I love Spark NLP""")
```
