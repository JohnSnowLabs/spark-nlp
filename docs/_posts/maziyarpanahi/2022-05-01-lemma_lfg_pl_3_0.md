...
Pretrained Lemmatizer model (`lemma_lfg`) trained on Universal Dependencies 2.9 (UD_Polish-LFG) in Polish language.
...
[Copy S3 URI](s3://auxdata.johnsnowlabs.com/public/models/lemma_lfg_pl_3.4.3_3.0_1651416586000.zip){:.button.button-orange.button-orange-trans.button-icon.button-copy-s3}
...
```python
import nlu
nlu.load("pl.lemma").predict("""I love Spark NLP""")
```
