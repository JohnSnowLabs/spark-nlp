...
Pretrained Lemmatizer model (`lemma_ftb`) trained on Universal Dependencies 2.9 (UD_Finnish-FTB) in Finnish language.
...
[Copy S3 URI](s3://auxdata.johnsnowlabs.com/public/models/lemma_ftb_fi_3.4.3_3.0_1651416620701.zip){:.button.button-orange.button-orange-trans.button-icon.button-copy-s3}
...
```python
import nlu
nlu.load("fi.lemma.ftb").predict("""I love Spark NLP""")
```
