...
Pretrained Lemmatizer model (`lemma_edt`) trained on Universal Dependencies 2.9 (UD_Estonian-EDT) in Estonian language.
...
[Copy S3 URI](s3://auxdata.johnsnowlabs.com/public/models/lemma_edt_et_3.4.3_3.0_1651416638052.zip){:.button.button-orange.button-orange-trans.button-icon.button-copy-s3}
...
```python
import nlu
nlu.load("et.lemma.edt").predict("""I love Spark NLP""")
```
