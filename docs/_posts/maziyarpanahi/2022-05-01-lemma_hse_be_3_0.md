...
Pretrained Lemmatizer model (`lemma_hse`) trained on Universal Dependencies 2.9 (UD_Belarusian-HSE) in Belarusian language.
...
[Copy S3 URI](s3://auxdata.johnsnowlabs.com/public/models/lemma_hse_be_3.4.3_3.0_1651429166013.zip){:.button.button-orange.button-orange-trans.button-icon.button-copy-s3}
...
```python
import nlu
nlu.load("be.lemma").predict("""I love Spark NLP""")
```
