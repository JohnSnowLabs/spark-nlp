...
```python
sentence_embeddings = RoBertaEmbeddings.pretrained("roberta_base_finnish_legal", "fi")\
  .setInputCols(["sentence"])\
  .setOutputCol("embeddings")
```
```scala
val sentence_embeddings = RoBertaEmbeddings.pretrained("roberta_base_finnish_legal", "fi")
  .setInputCols("sentence")
  .setOutputCol("embeddings"))
```
</div>
