...
```python
sentence_embeddings = RoBertaEmbeddings.pretrained("roberta_base_swedish_legal", "se")\
  .setInputCols(["sentence"])\
  .setOutputCol("embeddings")
```
```scala
val sentence_embeddings = RoBertaEmbeddings.pretrained("roberta_base_swedish_legal", "se")
  .setInputCols("sentence")
  .setOutputCol("embeddings"))
```
</div>
