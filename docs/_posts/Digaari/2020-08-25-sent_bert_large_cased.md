...
```python
...
embeddings = BertEmbeddings.pretrained("sent_bert_large_cased", "en") \
...
```scala
...
val embeddings = BertEmbeddings.pretrained("sent_bert_large_cased", "en")
      .setInputCols("sentence")
      .setOutputCol("sentence_embeddings")
...
text = ["I love NLP"]
embeddings_df = nlu.load('en.embed_sentence.bert_large_cased').predict(text, output_level='token')
embeddings_df
```
