...
spark-shell --jar spark-nlp-assembly-2.4.3
```
...
```bash
pip install spark-nlp-jsl==2.4.3 --extra-index-url #### --ignore-installed
```
...
From Licensed version 2.4.3 in order to access private JohnSnowLabs
...
We will be using version 2.4.3 of Spark NLP Open Source and 2.4.3 of
...
spark = SparkSession.builder \
    .appName("Global DEMO - Spark NLP Enterprise 2.4.3") \
    .master("local[*]") \
...
```bash
// https://mvnrepository.com/artifact/com.johnsnowlabs.nlp/spark-nlp
libraryDependencies += "com.johnsnowlabs.nlp" %% "spark-nlp" % "2.4.3"
```
...
5- Now, all available **Spark Packages** are at your fingertips! Just search for **JohnSnowLabs:spark-nlp:version** where **version** stands for the library version such as: `2.4.3`
