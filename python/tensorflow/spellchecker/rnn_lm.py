...
class RNNLM(object):
    ...
    def __init__(self,
        ...
        class_logits = tf.reshape(class_logits, [-1, self.num_classes])#(total_word_cnt, n_classes)
        ...
        cand_cnt = self.candidate_class_ids.shape[1]

        # broadcasted class logits - take only the last element, and repeat it
        bc_cl_logits = tf.tile(class_logits[-1, :], cand_cnt)
        ...
        bc_id_logits = tf.tile(wordid_logits[-1, :], cand_cnt)
    ...
    def predict(self, sess, raw_sentences, verbose=False):
        ...
        for raw_line in raw_sentences:
            ...
            sess.run(self.test_init_op, {self.in_memory_test: np.array([[wids, cids, wcids], [wids, cids, wcids]])})
    ...
    def predict_(self, sess, candidates, verbose=False):
        ...
        sent = 'she came to me in an unexpected'
        splits = sent.split()
...
class RNNLM(object):
    ...
    def __init__(self,
        ...
        # broadcasted class logits - take only the last element, and repeat it
        bc_cl_logits = tf.tile(class_logits[-1:, :], tf.reverse(cand_cnt, axis=tf.constant([0])))
        classid_losses = tf.nn.sparse_softmax_cross_entropy_with_logits \
                      (labels=tf.reshape(self.candidate_class_ids, [-1]), logits=bc_cl_logits, name='cidlosses') \
                         * tf.cast(tf.reshape(non_zero_weights, [-1]), tf.float32)
    ...
    def predict_(self, sess, candidates, verbose=False):
        ...
        losses = sess.run(
            ...
            {self.dropout_rate: 1.0,
             ...
             contextIds: [cids[1:]],
             contextWordIds: [wcids[1:]],
             self.candidate_word_ids: can_wcids,
             self.candidate_class_ids: can_cids
             })
