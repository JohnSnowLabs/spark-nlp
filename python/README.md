...
Spark NLP 5.4.1 is built with ONNX 1.17.0 and TensorFlow 2.7.1 deep learning engines. The minimum following NVIDIA® software are only required for GPU support:
...
$ pip install spark-nlp==5.4.1 pyspark==3.3.1
```
...
Spark NLP *5.4.1* has been built on top of Apache Spark 3.4 while fully supports Apache Spark 3.0.x, 3.1.x, 3.2.x, 3.3.x, 3.4.x, and 3.5.x
...
Spark NLP 5.4.1 has been tested and is compatible with the following runtimes:
...
Spark NLP 5.4.1 has been tested and is compatible with the following EMR releases:
...
spark-submit --packages com.johnsnowlabs.nlp:spark-nlp-gpu_2.12:5.4.1
...
spark-submit --packages com.johnsnowlabs.nlp:spark-nlp-aarch64_2.12:5.4.1
...
spark-submit --packages com.johnsnowlabs.nlp:spark-nlp-silicon_2.12:5.4.1
...
spark-shell \
  ...
  --packages com.johnsnowlabs.nlp:spark-nlp_2.12:5.4.1
```
...
```sbtshell
// https://mvnrepository.com/artifact/com.johnsnowlabs.nlp/spark-nlp
libraryDependencies += "com.johnsnowlabs.nlp" %% "spark-nlp" % "5.4.1"
```
...
```sbtshell
// https://mvnrepository.com/artifact/com.johnsnowlabs.nlp/spark-nlp-aarch64
libraryDependencies += "com.johnsnowlabs.nlp" %% "spark-nlp-aarch64" % "5.4.1"
```
...
```sbtshell
// https://mvnrepository.com/artifact/com.johnsnowlabs.nlp/spark-nlp-silicon
libraryDependencies += "com.johnsnowlabs.nlp" %% "spark-nlp-silicon" % "5.4.1"
```
...
```bash
com.johnsnowlabs.nlp:spark-nlp_2.12:5.4.1
```
...
$ conda activate sparknlp
# spark-nlp by default is based on pyspark 3.x
$ pip install spark-nlp==5.4.1 pyspark==3.3.1 jupyter
$ jupyter notebook
```
...
!wget https://setup.johnsnowlabs.com/colab.sh -O - | bash /dev/stdin -p 3.2.3 -s 5.4.1
```
...
3. In `Libraries` tab inside your cluster you need to follow these steps:
   ...
   3.1. Install New -> PyPI -> `spark-nlp==5.4.1` -> Install

   3.2. Install New -> Maven -> Coordinates -> `com.johnsnowlabs.nlp:spark-nlp_2.12:5.4.1` -> Install
...
},
{
  "Classification": "spark-defaults",
    "Properties": {
      ...
      "spark.jars.packages": "com.johnsnowlabs.nlp:spark-nlp_2.12:5.4.1"
    }
}]
```
...
gcloud dataproc clusters create ${CLUSTER_NAME} \
  ...
  --properties spark:spark.serializer=org.apache.spark.serializer.KryoSerializer,spark:spark.driver.maxResultSize=0,spark:spark.kryoserializer.buffer.max=2000M,spark:spark.jars.packages=com.johnsnowlabs.nlp:spark-nlp_2.12:5.4.1
```
...
spark = SparkSession.builder
    ...
    .config("spark.driver.maxResultSize", "0")
    .config("spark.kryoserializer.buffer.max", "2000M")
    .config("spark.jars", "/tmp/spark-nlp-assembly-5.4.1.jar")
    .getOrCreate()
```
...
- If you are local, you can load the Fat JAR from your local FileSystem, however, if you are in a cluster setup you need
  ...
  i.e., `hdfs:///tmp/spark-nlp-assembly-5.4.1.jar`)
