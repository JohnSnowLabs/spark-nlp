...
class PushToHub:
    ...
    def create_docs(dicionary_for_upload:dict) -> dict:
        ...
        if 'description' not in dicionary_for_upload.keys():
            dicionary_for_upload['description'] =f"This model is ussed for {dicionary_for_upload['task']} and this model works with {dicionary_for_upload['language']} language"
        
        if 'title' not in dicionary_for_upload.keys():
            dicionary_for_upload['title'] =f"{dicionary_for_upload['task']} for {dicionary_for_upload['language']} language"

        if os.path.isdir(dicionary_for_upload['model_zip_path']) : 
            PushToHub.zip_directory(dicionary_for_upload['model_zip_path'],f"{dicionary_for_upload['model_zip_path']}.zip")
