...
Spark NLP 4.1.0 is built with TensorFlow 2.7.1 and the following NVIDIA® software are only required for GPU support:
...
Spark NLP *4.1.0* has been built on top of Apache Spark 3.2 while fully supports Apache Spark 3.0.x, 3.1.x, 3.2.x, and 3.3.x:
...
spark-submit --packages com.johnsnowlabs.nlp:spark-nlp-gpu_2.12:4.1.0
...
spark-submit --packages com.johnsnowlabs.nlp:spark-nlp-m1_2.12:4.1.0
...
spark-shell \
  ...
  --packages com.johnsnowlabs.nlp:spark-nlp_2.12:4.1.0
```
...
$ conda activate sparknlp
# spark-nlp by default is based on pyspark 3.x
$ pip install spark-nlp==4.1.0 pyspark==3.2.1 jupyter
$ jupyter notebook
```
...
3. In `Libraries` tab inside your cluster you need to follow these steps:
    ...
    3.1. Install New -> PyPI -> `spark-nlp==4.1.0` -> Install
...
gcloud dataproc clusters create ${CLUSTER_NAME} \
  ...
  --properties spark:spark.serializer=org.apache.spark.serializer.KryoSerializer,spark:spark.driver.maxResultSize=0,spark:spark.kryoserializer.buffer.max=2000M,spark:spark.jars.packages=com.johnsnowlabs.nlp:spark-nlp_2.12:4.1.0
```
