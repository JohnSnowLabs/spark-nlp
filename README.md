...
Spark NLP *4.0.2* has been built on top of Apache Spark 3.2 while fully supports Apache Spark 3.0.x, 3.1.x, 3.2.x, and 3.3.x:
...
- Starting 4.0.2 release, the default `spark-nlp` and `spark-nlp-gpu` packages are based on Scala 2.12.15 and Apache Spark 3.2 by default.
...
Spark NLP 4.0.2 has been tested and is compatible with the following runtimes:
...
Spark NLP 4.0.2 has been tested and is compatible with the following EMR releases:
...
spark-submit --packages com.johnsnowlabs.nlp:spark-nlp-gpu_2.12:4.0.2
...
spark-submit --packages com.johnsnowlabs.nlp:spark-nlp-m1_2.12:4.0.2
...
spark-shell \
  ...
  --packages com.johnsnowlabs.nlp:spark-nlp_2.12:4.0.2
```
...
```bash
com.johnsnowlabs.nlp:spark-nlp_2.12:4.0.2
```
...
!wget https://setup.johnsnowlabs.com/colab.sh -O - | bash /dev/stdin -p 3.2.1 -s 4.0.2
```
...
},
{
  "Classification": "spark-defaults",
    "Properties": {
      ...
      "spark.jars.packages": "com.johnsnowlabs.nlp:spark-nlp_2.12:4.0.2"
    }
}]
```
...
- If you are local, you can load the Fat JAR from your local FileSystem, however, if you are in a cluster setup you need to put the Fat JAR on a distributed FileSystem such as HDFS, DBFS, S3, etc. (i.e., `hdfs:///tmp/spark-nlp-assembly-4.0.2.jar`)
