...
Spark NLP 4.2.0 is built with TensorFlow 2.7.1 and the following NVIDIA® software are only required for GPU support:
...
$ pip install spark-nlp==4.2.0 pyspark==3.2.1
```
...
Spark NLP *4.2.0* has been built on top of Apache Spark 3.2 while fully supports Apache Spark 3.0.x, 3.1.x, 3.2.x, and 3.3.x:
...
Spark NLP 4.2.0 has been tested and is compatible with the following runtimes:
...
Spark NLP 4.2.0 has been tested and is compatible with the following EMR releases:
...
spark-submit --packages com.johnsnowlabs.nlp:spark-nlp-gpu_2.12:4.2.0
...
spark-shell --packages com.johnsnowlabs.nlp:spark-nlp-aarch64_2.12:4.2.0

pyspark --packages com.johnsnowlabs.nlp:spark-nlp-aarch64_2.12:4.2.0

spark-submit --packages com.johnsnowlabs.nlp:spark-nlp-aarch64_2.12:4.2.0
...
spark-submit --packages com.johnsnowlabs.nlp:spark-nlp-m1_2.12:4.2.0
...
spark-shell \
  ...
  --packages com.johnsnowlabs.nlp:spark-nlp_2.12:4.2.0
```
...
```sbtshell
// https://mvnrepository.com/artifact/com.johnsnowlabs.nlp/spark-nlp
libraryDependencies += "com.johnsnowlabs.nlp" %% "spark-nlp" % "4.2.0"
```
...
```sbtshell
// https://mvnrepository.com/artifact/com.johnsnowlabs.nlp/spark-nlp-aarch64
libraryDependencies += "com.johnsnowlabs.nlp" %% "spark-nlp-aarch64" % "4.2.0"
```
...
```sbtshell
// https://mvnrepository.com/artifact/com.johnsnowlabs.nlp/spark-nlp-m1
libraryDependencies += "com.johnsnowlabs.nlp" %% "spark-nlp-m1" % "4.2.0"
```
...
!wget https://setup.johnsnowlabs.com/colab.sh -O - | bash /dev/stdin -p 3.2.1 -s 4.2.0
```
...
3. In `Libraries` tab inside your cluster you need to follow these steps:
    ...
    3.1. Install New -> PyPI -> `spark-nlp==4.2.0` -> Install

    3.2. Install New -> Maven -> Coordinates -> `com.johnsnowlabs.nlp:spark-nlp_2.12:4.2.0` -> Install
...
gcloud dataproc clusters create ${CLUSTER_NAME} \
  ...
  --properties spark:spark.serializer=org.apache.spark.serializer.KryoSerializer,spark:spark.driver.maxResultSize=0,spark:spark.kryoserializer.buffer.max=2000M,spark:spark.jars.packages=com.johnsnowlabs.nlp:spark-nlp_2.12:4.2.0
```
...
spark = SparkSession.builder \
    ...
    .config("spark.driver.maxResultSize", "0") \
    .config("spark.kryoserializer.buffer.max", "2000M")\
    .config("spark.jars", "/tmp/spark-nlp-assembly-4.2.0.jar")\
    .getOrCreate()
```
...
- If you are local, you can load the Fat JAR from your local FileSystem, however, if you are in a cluster setup you need to put the Fat JAR on a distributed FileSystem such as HDFS, DBFS, S3, etc. (i.e., `hdfs:///tmp/spark-nlp-assembly-4.2.0.jar`)
