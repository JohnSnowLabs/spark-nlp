...
class NativeLibraryPreloaderTestSpec extends AnyFlatSpec with BeforeAndAfterEach {
  ...
  it should "apply independent configured and runtime path-list character budgets" taggedAs FastTest in {
    val configured = "/" + ("c" * (NativeLibraryPreloader.MaxConfiguredPathChars - 1))
    val runtime = "/" + ("r" * (NativeLibraryPreloader.MaxConfiguredPathChars - 1))

    val configuredPaths =
      NativeLibraryPreloader.PreloadConfig.parse("search", configured).paths
    val runtimePaths = NativeLibraryPreloader.parseRuntimePathLists(
      Seq(runtime),
      NativeLibraryPreloader.SearchLimits(maxDepth = 1, maxVisitedEntries = 2),
      NativeLibraryPreloader.MaxConfiguredPathChars)

    assert(configuredPaths == Seq(configured))
    assert(runtimePaths == Seq(Paths.get(runtime)))
  }
  ...
  it should "make a native load failure terminal before another manifest can be attempted" taggedAs FastTest in {
    val firstDirectory = tempDirectory()
    val firstPaths = createLibrarySet(firstDirectory)
    val secondDirectory = tempDirectory()
    val secondPaths = createLibrarySet(secondDirectory)
    val attempted = ArrayBuffer.empty[Path]
    var loadAttempts = 0
    val loader = new NativeLibraryPreloader.LibraryLoader {
      override def load(path: Path): Unit = {
        loadAttempts += 1
        attempted += path
        if (loadAttempts == 2) throw new UnsatisfiedLinkError("synthetic second-library failure")
      }
    }
    val preloader = new NativeLibraryPreloader(
      requiredLibraries,
      _ => NativeLibraryPreloader.SearchSources.empty,
    ...
    val firstFailure = intercept[IllegalStateException] {
      preloader.preload(
        NativeLibraryPreloader
          .PreloadConfig(NativeLibraryPreloader.Explicit, firstPaths.map(_.toString)))
    }
    val secondFailure = intercept[IllegalStateException] {
      preloader.preload(
        NativeLibraryPreloader
          .PreloadConfig(NativeLibraryPreloader.Explicit, secondPaths.map(_.toString)))
    }

    assert(firstFailure.getMessage.contains("synthetic second-library failure"))
    assert(secondFailure.getMessage.contains("terminal"))
    assert(attempted == firstPaths.take(2).map(_.toRealPath()))
  }
  ...
  it should "reject matching group-writable platform libraries discovered in search mode" taggedAs FastTest in {
    val directory = tempDirectory()
    val paths = createLibrarySet(directory)
    val groupWritableLibrary = paths.head.toRealPath()
    val platformPolicy = NativeLibraryPreloader.SecurityPolicy(
    ...
    val error = intercept[IllegalStateException] {
      testPreloader(ArrayBuffer.empty, securityPolicy = platformPolicy).preload(
        NativeLibraryPreloader
          .PreloadConfig(NativeLibraryPreloader.Search, Seq(directory.toString)))
    }
