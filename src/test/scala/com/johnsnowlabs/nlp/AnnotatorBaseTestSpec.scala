...
class DummyAnnotatorModel(override val uid: String)
    extends AnnotatorModel[DummyAnnotatorModel]
    with HasSimpleAnnotate[DummyAnnotatorModel] {
  ...
  override def annotate(annotations: Seq[Annotation]): Seq[Annotation] =
    Seq(Annotation(outputAnnotatorType, 0, 25, "dummy result", Map("a" -> "b", "c" -> "d")))
}
object DummyAnnotatorModel extends DefaultParamsReadable[DummyAnnotatorModel]

class DemandingDummyAnnotatorModel(override val uid: String)
    extends AnnotatorModel[DemandingDummyAnnotatorModel]
    with HasSimpleAnnotate[DemandingDummyAnnotatorModel] {
  import AnnotatorType._
  override val outputAnnotatorType: AnnotatorType = DUMMY
  ...
  override def annotate(annotations: Seq[Annotation]): Seq[Annotation] =
    Seq(Annotation(DUMMY, 11, 18, "dummy result", Map("aa" -> "bb", "cc" -> "dd")))
}
...
class AnnotatorBaseTestSpec extends AnyFlatSpec {
  ...
  "dummy annotators" should "transform data with default params" taggedAs FastTest in {
    ...
    assert(
      result.columns.contains(dummyAnnotator.getOutputCol) &&
  ...
  "dummy annotators" should "transform data with changed params" taggedAs FastTest in {
    ...
    val result = demandingDummyAnnotator.transform(dummyAnnotator.transform(dummyData))
    assert(
      result.columns.contains("demand") &&
  ...
  "dummy annotators" should "transform schema and reflect content metadata as well as schema metadata" taggedAs FastTest in {
    ...
    assert(
      demandContentAnnotation.metadata.contains("aa") && demandContentAnnotation.metadata(
        "aa") == "bb")
  }
  ...
  "demanding dummy annotator" should "fail if input columns are not found" taggedAs FastTest in {
    ...
    assertThrows[IllegalArgumentException](
      demandingDummyAnnotator.transform(dummyAnnotator.transform(dummyData)))
  }
