...
class WordEmbeddingsWithGlossaryTestSpec extends FlatSpec {
  ...
  val dummyDataset = spark.createDataset(Seq.empty[String]).toDF("text")
  ...
  "Training NERModel" should "train model" in {
    ...
    val readyData = embeddings.transform(trainingDataset).cache()


    println("newTagger definition")
...
if(false) {
  "ClinicalWordEmbeddings" should "deidentify better with a surname glossary" in {
    ...
    val srcDF = Seq(
      "Record date : 2093-01-13 Team X Intern Admission Note Name : Hendrickson , Ora MR # 7194334 Date : 01/13/93 PCP : Oliveira , Keith MD CC/RFA : SOB .\n\nHPI : Pt is a 76 yo F with a h/o CAD , HTN , hypercholesterolemia , COPD , CHF who developed acute SOB while at home PM of admission .\n\nPt reports no dietary discretions and excellent adherence to medications .\n\nShe was out evening of admission at a social gathering and noticed symptoms when she returned home and had difficulty climbing an internal staircase in her home She ascended the staircase and was unable to regain her breath .",
      "Ionnidis Papaloudus")
      .toDF("text")
