...
class GPTJTestSpec extends AnyFlatSpec {
  "gpt2" should "should handle temperature=0 correctly and not crash when predicting more than 1 element with doSample=True" in {
    ...
    val gpt2 = GPTJTransformer
      .loadSavedModel(
        "/research/Phd/SparkNLP/GPTJ/anton-l/gpt-j-tiny-random/saved_model/1",
  ...
  "gpt2" should "run SparkNLP pipeline with larger batch size" in {
    val testData = ResourceHelper.spark
      .createDataFrame(Seq(
        (1, "My name is Leonardo."),
        (2, "My name is Leonardo and I come from Rome."),
