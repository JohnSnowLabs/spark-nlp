...
trait PerceptronApproachBehaviors { this: FlatSpec =>
  ...
  def sparkBasedPOSTagger(dataset: => Dataset[Row]): Unit = {
    ...
    it should "tag each word sentence" in {
      ...
      val tokens: Seq[String] = tokensCol.collect.flatMap(r => r.getSeq[Row](0)).flatMap(a => a.getMap[String, String](2).get("token")).map(_.toString)
      val taggedWords: Seq[String] = posCol.collect.flatMap(r => r.getSeq[Row](0)).flatMap(a => a.getMap[String, String](2).get("word")).map(_.toString)
    ...
    it should "annotate with the correct word index" in {
      ...
      val posCol = df.select("pos")
      val tokensCol = df.select("token")
      val tokens: Seq[IndexedWord] = tokensCol.collect.flatMap(r => r.getSeq[Row](0)).map(a => IndexedWord(a.getString(1), a.getMap[String, String](2)(Annotation.BEGIN).toInt, a.getMap[String, String](2)(Annotation.END).toInt))
      val taggedWords: Seq[IndexedWord] = posCol.collect.flatMap(r => r.getSeq[Row](0)).map(a => IndexedWord(a.getMap[String, String](2)("word"), a.getMap[String, String](2)(Annotation.BEGIN).toInt, a.getMap[String, String](2)(Annotation.END).toInt))
