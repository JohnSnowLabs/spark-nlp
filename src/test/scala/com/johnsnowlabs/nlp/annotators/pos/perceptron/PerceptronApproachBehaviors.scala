...
trait PerceptronApproachBehaviors { this: FlatSpec =>
  ...
  def sparkBasedPOSTagger(dataset: => Dataset[Row]): Unit = {
    ...
    it should "annotate with the correct word index" in {
      ...
      val posCol = df.select("pos")
      val tokensCol = df.select("token")
      val tokens: Seq[IndexedWord] = tokensCol.collect.flatMap(r => r.getSeq[Row](0)).map(a => IndexedWord(a.getMap[String, Any](4).get("token").get.toString, a.getInt(1), a.getInt(2)))
      val taggedWords: Seq[IndexedWord] = posCol.collect.flatMap(r => r.getSeq[Row](0)).map(a => IndexedWord(a.getMap[String, Any](4).get("word").get.toString, a.getInt(1), a.getInt(2)))
