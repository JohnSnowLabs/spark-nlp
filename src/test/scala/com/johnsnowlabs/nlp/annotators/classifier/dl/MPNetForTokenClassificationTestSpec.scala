...
class MPNetForTokenClassificationTestSpec extends AnyFlatSpec {

  import ResourceHelper.spark.implicits._

  "MPNetForTokenClassification" should "run end to end pipeline test" taggedAs SlowTest in {
  ...
  "MPNetForTokenClassification" should "be saved and loaded correctly" taggedAs LocalTest in {
    ...
    Benchmark.time("Time to save BertForTokenClassification model") {
      pipelineModel.stages.last
        .asInstanceOf[MPNetForTokenClassification]
        .write
        .overwrite()
        .save("./tmp_mpnetfortoken_model")
    }

    val loadedPipelineModel = PipelineModel.load("./tmp_mpnetfortoken_pipeline")
    loadedPipelineModel.transform(ddd).select("label.result").show(false)

    val loadedSequenceModel =
      MPNetForTokenClassification.load("./tmp_mpnetfortoken_model")
  ...
  "MPNetForTokenClassification" should "benchmark test" taggedAs LocalTest in {
    ...
    Benchmark.time("Time to save the results") {
      pipelineDF.write.mode("overwrite").parquet("./tmp_mpnet_token_classifier")
    }
