...
class XlmRoBertaForZeroShotClassificationTestSpec extends AnyFlatSpec {
  ...
  "XlmRoBertaForZeroShotClassification" should "correctly load custom model with extracted signatures" taggedAs SlowTest in {
  ...
  "XlmRoBertaForZeroShotClassification" should "be saved and loaded correctly" taggedAs SlowTest in {
    ...
    Benchmark.time("Time to save XlmRoBertaForZeroShotClassification pipeline model") {
      pipelineModel.write.overwrite().save("./tmp_xlmrobertafornli_pipeline")
    }

    Benchmark.time("Time to save XlmRoBertaForZeroShotClassification model") {
      pipelineModel.stages.last
        .asInstanceOf[XlmRoBertaForZeroShotClassification]
        .write
        .overwrite()
        .save("./tmp_xlmrobertafornli_model")
    }

    val loadedPipelineModel = PipelineModel.load("./tmp_xlmrobertafornli_pipeline")
    loadedPipelineModel.transform(ddd).select("label.result").show(false)

    val loadedSequenceModel =
      XlmRoBertaForZeroShotClassification.load("./tmp_xlmrobertafornli_model")
    println(loadedSequenceModel.getClasses.mkString("Array(", ", ", ")"))

  }

  "XlmRoBertaForZeroShotClassification" should "benchmark test" taggedAs SlowTest in {
