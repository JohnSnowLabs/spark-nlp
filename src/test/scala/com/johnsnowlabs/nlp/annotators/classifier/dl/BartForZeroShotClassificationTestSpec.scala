...
class BartForZeroShotClassificationTestSpec extends AnyFlatSpec {
  ...
  "BartForZeroShotClassification" should "be saved and loaded correctly" taggedAs SlowTest in {
    ...
    Benchmark.time("Time to save BartForZeroShotClassification pipeline model") {
      pipelineModel.write.overwrite().save("./tmp_bartfornli_pipeline")
    }

    Benchmark.time("Time to save BartForZeroShotClassification model") {
      pipelineModel.stages.last
        .asInstanceOf[BartForZeroShotClassification]
        .write
        .overwrite()
        .save("./tmp_bartfornli_model")
    }

    val loadedPipelineModel = PipelineModel.load("./tmp_Bartfornli_pipeline")
    loadedPipelineModel.transform(ddd).select("label.result").show(false)

    val loadedSequenceModel =
      BartForZeroShotClassification.load("./tmp_bartfornli_model")
  ...
  "BartForZeroShotClassification" should "Load test" taggedAs SlowTest in {
