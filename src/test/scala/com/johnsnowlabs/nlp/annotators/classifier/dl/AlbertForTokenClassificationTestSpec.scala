...
class AlbertForTokenClassificationTestSpec extends AnyFlatSpec {

  import ResourceHelper.spark.implicits._

  "AlbertForTokenClassification" should "correctly load custom model with extracted signatures" taggedAs SlowTest in {
  ...
  "AlbertForTokenClassification" should "be saved and loaded correctly" taggedAs SlowTest in {
    ...
    Benchmark.time("Time to save AlbertForTokenClassification pipeline model") {
      pipelineModel.write.overwrite().save("./tmp_albertfortoken_pipeline")
    }

    Benchmark.time("Time to save AlbertForTokenClassification model") {
      pipelineModel.stages.last.asInstanceOf[AlbertForTokenClassification].write.overwrite().save("./tmp_albertfortoken_model")
    }

    val loadedPipelineModel = PipelineModel.load("./tmp_albertfortoken_pipeline")
  ...
  "AlbertForTokenClassification" should "benchmark test" taggedAs SlowTest in {
    ...
    Benchmark.time("Time to save the results") {
      pipelineDF.write.mode("overwrite").parquet("./tmp_albert_token_classifier")
    }
