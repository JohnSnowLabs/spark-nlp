...
class BertForMultipleChoiceTestSpec extends AnyFlatSpec with SparkSessionTest {
  ...
  "BertForMultipleChoiceTestSpec" should "run end to end pipeline test" taggedAs SlowTest in {
    val resultDf = pipelineModel.transform(testDataframe)
    resultDf.show(truncate = false)
  }
  ...
  private def getBertForMultipleChoicePipelineModel = {
