...
class RoBertaForZeroShotClassificationTestSpec extends AnyFlatSpec {
  ...
  "RoBertaForZeroShotClassification" should "run end to end pipeline test" taggedAs SlowTest in {
  ...
  "RoBertaForZeroShotClassification" should "correctly load custom model with extracted signatures" taggedAs LocalTest in {
  ...
  "RoBertaForZeroShotClassification" should "be saved and loaded correctly" taggedAs LocalTest in {
    ...
    Benchmark.time("Time to save RoBertaForZeroShotClassification pipeline model") {
      pipelineModel.write.overwrite().save("./tmp_robertafornli_pipeline")
    }

    Benchmark.time("Time to save RoBertaForZeroShotClassification model") {
      pipelineModel.stages.last
        .asInstanceOf[RoBertaForZeroShotClassification]
        .write
        .overwrite()
        .save("./tmp_robertafornli_model")
    }

    val loadedPipelineModel = PipelineModel.load("./tmp_robertafornli_pipeline")
    loadedPipelineModel.transform(ddd).select("label.result").show(false)

    val loadedSequenceModel =
      RoBertaForZeroShotClassification.load("./tmp_robertafornli_model")
    println(loadedSequenceModel.getClasses.mkString("Array(", ", ", ")"))

  }

  "RoBertaForZeroShotClassification" should "benchmark test" taggedAs LocalTest in {
