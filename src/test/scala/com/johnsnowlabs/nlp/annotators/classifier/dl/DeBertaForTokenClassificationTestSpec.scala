...
class DeBertaForTokenClassificationTestSpec extends AnyFlatSpec {

  import ResourceHelper.spark.implicits._

  "DeBertaForTokenClassification" should "run end to end pipeline test" taggedAs SlowTest in {
    ...
    val tokenClassifier = DeBertaForTokenClassification
      .pretrained()
  ...
  "DeBertaForTokenClassification" should "correctly load custom model with extracted signatures" taggedAs LocalTest in {
  ...
  "DeBertaForTokenClassification" should "be saved and loaded correctly" taggedAs LocalTest in {
    ...
    Benchmark.time("Time to save DeBertaForTokenClassification pipeline model") {
      pipelineModel.write.overwrite().save("./tmp_debertafortoken_pipeline")
    }

    Benchmark.time("Time to save DeBertaForTokenClassification model") {
      pipelineModel.stages.last
        .asInstanceOf[DeBertaForTokenClassification]
        .write
        .overwrite()
        .save("./tmp_debertafortoken_model")
    }

    val loadedPipelineModel = PipelineModel.load("./tmp_debertafortoken_pipeline")
    loadedPipelineModel.transform(ddd).select("label.result").show(false)

    val loadedSequenceModel = DeBertaForTokenClassification.load("./tmp_debertafortoken_model")
    println(loadedSequenceModel.getClasses.mkString("Array(", ", ", ")"))

  }

  "DeBertaForTokenClassification" should "benchmark test" taggedAs LocalTest in {
    ...
    Benchmark.time("Time to save DeBertaForTokenClassification results") {
