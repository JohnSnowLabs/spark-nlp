...
class NormalizerTestSpec extends AnyFlatSpec with NormalizerBehaviors {
  ...
  "A full Normalizer pipeline with latin content" should behave like fullNormalizerPipeline(
    latinBodyData)
  "A Normalizer pipeline with latin content and disabled lowercasing" should behave like lowercasingNormalizerPipeline(
    latinBodyData)
  ...
  private var data = Seq(
    ...
    ("b4", "before"),
    ("4", "for"),
    ("Yo dude whatsup?", "hey@friend@how@are@you")).toDS.toDF("text", "normalized_gt")

  "an isolated normalizer " should behave like testCorrectSlangs(data)

  data = Seq(("test-ing", "testing"), ("test-ingX", "testing")).toDS.toDF("text", "normalized_gt")
