...
/** Integration tests for [[SentenceDetectorSaTModel]].
  ...
  * as `model.onnx` + `assets/sentencepiece.bpe.model`, expected in the folder `"1"` at the
  * repository root – hence they are tagged [[SlowTest]].
  */
class SentenceDetectorSaTSpec extends AnyFlatSpec with Matchers {
  ...
  private val longEnglish: String =
    """The landscape of modern software engineering is undergoing a profound paradigm shift. For
      ...
      |design, and execute software across the globe.""".stripMargin
  private val longGerman: String =
    """Die Landschaft der modernen Softwareentwicklung befindet sich in einem tiefgreifenden Paradigmenwechsel.
      ...
      |vollzogen und die Art und Weise, wie wir weltweit Software konzipieren, entwerfen und ausführen, permanent verändert.""".stripMargin
  ...
  "SentenceDetectorSaTModel" should "segment a long English document (>500 words)" taggedAs SlowTest in {
    val sentences = segment(longEnglish, loadModel())
    sentences.length should be >= 2
  }

  "SentenceDetectorSaTModel" should "segment a long German document (>500 words)" taggedAs SlowTest in {
    val sentences = segment(longGerman, loadModel())
    sentences.length should be >= 2
  }

  "SentenceDetectorSaTModel" should "respect min/max sentence length bounds" taggedAs SlowTest in {
    val minLen = 150
    val maxLen = 305

    val model = loadModel()
      .setMinSentenceLength(minLen)
      .setMaxSentenceLength(maxLen)
      .setTrimWhitespace(false)

    val sentences = segment(longEnglish, model)
    sentences.zipWithIndex.foreach { case (s, i) =>
      println(s"Sentence ${i + 1} (length ${s.length}): $s")
    }
    all(sentences.map(_.length).toSeq) should (be >= minLen and be <= maxLen)
  }
}
