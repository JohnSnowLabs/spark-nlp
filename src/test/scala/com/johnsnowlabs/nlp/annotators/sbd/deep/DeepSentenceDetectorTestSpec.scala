...
class DeepSentenceDetectorTestSpec extends FlatSpec with DeepSentenceDetectorBehaviors {
  ...
  if (SystemUtils.IS_OS_WINDOWS)
    ...
    val strongNerDataSet =
    nerReader.readDataset(SparkAccessor.spark, "src/test/resources/ner-corpus/sentence-detector/mix_dataset.txt")
    val weakNerDataSet =
    nerReader.readDataset(SparkAccessor.spark, "src/test/resources/ner-corpus/sentence-detector/unpunctuated_dataset.txt")

    var paragraph = ""
    var documentAnnotation =
      Seq[Annotation]()
    var pragmaticSegmentedSentences =
      Seq[Annotation]()
    var unpunctuatedSentences =
      Seq[Annotation]()
    var validNerEntities =
      Seq[Seq[Annotation]]()
    ...
    val strongPipeline =
    new RecursivePipeline().setStages(
    ...
    def getAnnotations(dataset: Dataset[_], pipeline: RecursivePipeline, column: String)
    ...
    def getNerTagger(trainingFile: String): NerDLModel
