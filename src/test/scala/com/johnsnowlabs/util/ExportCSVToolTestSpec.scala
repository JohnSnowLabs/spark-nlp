...
class ExportCSVToolTestSpec extends AnyFlatSpec with ExportCSVToolBehaviors {
  ...
  val document =
    Seq("EU rejects German call to boycott British lamb", "Peter Blackburn").toDS.toDF("text")
  ...
  val document2 = Seq(
    (
      ...
      Array(("sentence", "1"), ("sentence", "1"), ("sentence", "2"), ("sentence", "2")))).toDS
    .toDF("token", "pos", "result", "metadata")
