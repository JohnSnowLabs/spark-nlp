...
class TensorflowXlnet(val tensorflow: TensorflowWrapper,
  ...
  private val specialTokenSEPId = Array(4145, 83, 8186, 3158)
  private val specialTokenCLSId = Array(4145, 7416, 83, 3158)
  ...
  def tag(batch: Seq[Array[Int]]): Seq[Array[Array[Float]]] = {
    ...
    batch.map { tokenIds =>
      ...
      if (tokenIds.length >= maxSentenceLength) {
        ...
        maskBuffers.put(tokenIds.map(x=> if (x == 0.0f) 0 else 1.0f))
      }
      else {
        val newTokenIds = tokenIds ++ Array.fill(1, diff)(0).head
        tokenBuffers.put(newTokenIds)
        maskBuffers.put(newTokenIds.map(x=> if (x == 0.0f) 0 else 1.0f))
      }
    }
  ...
  def calculateEmbeddings(sentences: Seq[TokenizedSentence],
    ...
    sentences.grouped(batchSize).toArray.flatMap { batch =>
      ...
      val tokenIds = tokensPiece.map { sentence =>
        sentence.flatMap(x => x.tokens.map(x => x.pieceId)) ++ specialTokenSEPId ++ specialTokenCLSId
      }
      val vectors = tag(tokenIds)
      val t = vectors.length
      t
