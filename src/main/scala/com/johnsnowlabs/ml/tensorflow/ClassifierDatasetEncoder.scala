...
class ClassifierDatasetEncoder(val params: ClassifierDatasetEncoderParams) extends Serializable {
  ...
  def encodeTagsMultiLabel(labels: Array[Array[String]]): Array[Array[Float]] = {
    labels.map { t =>
      ...
      if (t.length > 0)
        t.foreach(x => labelIDsArray(tags2Id(x)) = 1.0f)
      labelIDsArray
    }
  }
   ...
   */
  def collectTrainingInstances(
      dataset: DataFrame,
      labelCol: String): Array[Array[(String, Array[Float])]] = {
    val results = dataset
      ...
      .map { row =>
        val newRow = row
          .get(0)
          .asInstanceOf[mutable.WrappedArray[mutable.WrappedArray[Float]]]
          .map(x => x.toArray)
   ...
   */
  def collectTrainingInstancesMultiLabel(
      dataset: DataFrame,
      labelCol: String): Array[Array[(Array[String], Array[Float])]] = {
    val results = dataset
      .select("embeddings", labelCol)
   ...
   */
  def extractSentenceEmbeddings(
      dataset: Array[Array[(String, Array[Float])]]): Array[Array[Float]] = {
    dataset.flatMap { x =>
      x.map { x =>
   ...
   */
  def extractSentenceEmbeddingsMultiLabel(
      dataset: Array[Array[(Array[String], Array[Float])]]): Array[Array[Array[Float]]] = {
    dataset.flatMap { x =>
      x.groupBy(x => x._1).map { x =>
        x._2.map { y =>
   ...
   */
  def extractSentenceEmbeddingsMultiLabel(
      docs: Seq[(Int, Seq[Annotation])]): Array[Array[Array[Float]]] = {
    docs
      .groupBy(x => x._1)
      .map { x =>
  ...
  def extractSentenceEmbeddingsMultiLabelPredict(
      docs: Seq[(Int, Seq[Annotation])]): Array[Array[Array[Float]]] = {
    Array(
      docs
        .flatMap(x =>
   ...
   * @return Array of Array of String
   */
  def extractLabelsMultiLabel(
      dataset: Array[Array[(Array[String], Array[Float])]]): Array[Array[String]] = {
    dataset.flatMap { x =>
  ...
  def calculateEmbeddingsDim(dataset: DataFrame): Int = {
    val embedSize = dataset
      .select(explode(col("embeddings")).as("embedding"))
      .select(size(col("embedding")).as("embeddings_size"))
      .rdd
      .take(1)
      .map(r => r.getInt(0))
    embedSize(0)
