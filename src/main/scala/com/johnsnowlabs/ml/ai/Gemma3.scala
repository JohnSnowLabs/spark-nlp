...
private[johnsnowlabs] class Gemma3(
    val onnxWrappers: Option[DecoderWrappers],
    val openvinoWrapper: Option[Gemma3Wrappers],
  ...
  val bpeTokenizer: Gemma3Tokenizer = BpeTokenizer
    .forModel(
      "gemma3",
      merges = merges,
      vocab = vocabulary,
  ...
  def encodeText(sentences: Seq[Annotation], imgTokenLen: List[Int]): Seq[Array[Int]] = {
    ...
    val pattern = raw"<start_of_image>".r

    // raise an error if the pattern is not found in the text
    if (pattern.findFirstIn(sentences.head.result).isEmpty) {
      throw new IllegalArgumentException("The pattern <start_of_image> is not found in the text")
    }
  ...
  def getModelOutputs(
    ...
    inferRequestLanguageModel.set_tensor("token_type_ids", tokenTypeTensor)
