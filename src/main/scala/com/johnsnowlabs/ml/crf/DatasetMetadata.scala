...
class DatasetMetadata(
  ...
  require(
    attrFeatures.length + transitions.length == featuresStat.length,
  ...
  for (i <- attrs.indices)
    require(
      attrs(i).id == i,
      s"Attribute ${attrs(i)} stored at index $i that does not equal to id")

  for (i <- attrFeatures.indices)
    require(
      attrFeatures(i).id == i,
      s"Feature ${attrFeatures(i)} stored at index $i that does not equal to id")
  ...
  lazy val attrFeatures2Id: Map[(Int, Int), Int] =
    attrFeatures.map(f => ((f.attrId, f.label), f.id)).toMap

  // Transition -> Feature Id
  lazy val transFeature2Id: Map[Transition, Int] = {
    transitions.zipWithIndex
  ...
  def filterFeatures(featureIds: Seq[Int]): DatasetMetadata = {
    ...
    val filteredTransFeatures =
      transFeaturesIds.map(id => transitions(id - attrFeatures.length)).toArray
