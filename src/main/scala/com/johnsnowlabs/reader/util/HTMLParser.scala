...
object HTMLParser {
   ...
   *
   * @param style CSS style string (e.g., "font-size: 1.2em; ...")
   * @param baseEmPx Base pixel size for 1em (default 16)
   * @param baseRemPx Base pixel size for 1rem (default 16)
   * @param parentPx Parent font size for '%' (default 16)
   * @return Font size in pixels, or 0 if not found
   */
  def extractFontSize(
                       style: String,
                       baseEmPx: Int = 16,
  ...
  def isFormattedAsTitle(style: String, titleFontSize: Int): Boolean = {
    ...
    val isLargeFont = lowerStyle.contains("font-size") && extractFontSize(lowerStyle) >= titleFontSize
