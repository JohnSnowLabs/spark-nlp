...
class SparkNLPReader(params: java.util.Map[String, String] = new java.util.HashMap()) {
  ...
  def html(htmlPath: String): DataFrame = {
    val htmlReader = new HTMLReader(getTitleFontSize(16))
  ...
  private def getTitleFontSize(defaultValue: Int): Int = {
    val titleFontSize =
      try {
        params.asScala.getOrElse("titleFontSize", defaultValue.toString).toInt
      } catch {
  ...
  def xls(docPath: String): DataFrame = {
    val excelReader = new ExcelReader(getTitleFontSize(9), getCellSeparator)
