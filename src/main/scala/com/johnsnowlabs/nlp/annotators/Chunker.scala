...
class Chunker(override val uid: String)
  ...
  override def annotate(annotations: Seq[Annotation]): Seq[Annotation] = {
    ...
    sentences.zipWithIndex.flatMap {
      case (sentence, sentenceIndex) =>
        val sentencePos = annotations.filter(
          pos =>
        ...
        val chunkPhrases = POSTagPatterns
          .flatMap(POSTagPattern =>
            getChunkPhrases(POSTagPattern, POSFormatSentence, sentencePos))
          .flatten

        val chunkAnnotations = chunkPhrases.zipWithIndex.map {
          case (phrase, idx) =>
