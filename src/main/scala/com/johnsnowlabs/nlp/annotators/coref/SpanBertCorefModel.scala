  ...
  override def annotate(annotations: Seq[Annotation]): Seq[Annotation] = {
    ...
    predictedClusters
      .flatMap(cluster => {
        ...
        // WordpieceEncoder-tokenized (see `tokenizedSentences`, built with
        // `WordpieceEncoder.encode` above), so the vocab merge strategy applies -- same
        // reconstruction RoBertaClassification/MPNetClassification/XXXForClassification use,
        // undoing the extra spaces a naive word-start-token join introduces around punctuation
        // and contractions (e.g. "Levi ' s Stadium" -> "Levi's Stadium").
