...
trait Tagged[T >: TaggedSentence <: TaggedSentence] extends Annotated[T] {
  ...
  def collectLabeledInstances(dataset: Dataset[Row],
    ...
    dataset
      ...
      .collect()
      .flatMap { row =>
        val labelAnnotations = getAnnotations(row, 0).toSeq
  ...
  def getAnnotations(row: Row, colNum: Int): scala.collection.Seq[Annotation] = {
    row.getAs[scala.collection.Seq[Row]](colNum).map(obj => Annotation(obj))
  }
  ...
  Array[(TextSentenceLabels, PosTaggedSentence, WordpieceEmbeddingsSentence)] = {
    ...
    annotations
      .flatMap { row =>
        ...
        val labels = getLabelsFromTaggedSentences(sentences, labelAnnotations.toSeq)
    ...
    new Iterator[Array[(TextSentenceLabels, WordpieceEmbeddingsSentence)]] {
      ...
      override def next(): Array[(TextSentenceLabels, WordpieceEmbeddingsSentence)] = {
        ...
        while (it.hasNext && count < batchSize) {
          ...
          val labels = getLabelsFromSentences(sentences, labelAnnotations.toSeq)
