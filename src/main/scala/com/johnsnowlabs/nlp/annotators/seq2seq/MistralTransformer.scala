    ...
    if (_model.isEmpty) {
      _model = Some(
        spark.sparkContext.broadcast(
          new Mistral(onnxWrappers, spp = spp, generationConfig = getGenerationConfig)))
    }
    this
  }
...
trait ReadMistralTransformerDLModel extends ReadOnnxModel with ReadSentencePieceModel {
  ...
  def loadSavedModel(modelPath: String, spark: SparkSession): MistralTransformer = {
