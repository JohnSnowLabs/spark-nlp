 ...
 * [[https://en.wikipedia.org/wiki/Part-of-speech_tagging Part-Of-Speech tags]] is required. The data has to be loaded
 * into a dataframe, where the column is an [[com.johnsnowlabs.nlp.Annotation Annotation]] of type `"POS"`. This can be
 * set with `setPosColumn`.
 *
 * '''Tip''': The helper class [[com.johnsnowlabs.nlp.training.POS POS]] might be useful to read training data into data frames.
 *
 * For extended examples of usage, see the [[https://github.com/JohnSnowLabs/spark-nlp-workshop/tree/master/jupyter/annotation/chinese/word_segmentation Spark NLP Workshop]]
