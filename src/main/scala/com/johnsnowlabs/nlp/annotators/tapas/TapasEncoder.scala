...
class TapasEncoder(val sentenceStartTokenId: Int,
                   val sentenceEndTokenId: Int,
                   encoder: WordpieceEncoder) {

  protected val NUMBER_PATTERN: Regex = "((^|\\s)[+-])?((\\.\\d+)|(\\d+(,\\d\\d\\d)*(\\.\\d*)?))".r
  ...
  protected val NUMBER_WORDS: Array[String] = Array("zero",  "one",  "two",  "three",  "four",  "five",
    "six",  "seven", "eight",  "nine",  "ten", "eleven", "twelve")
  protected val ORDINAL_WORDS: Array[String] = Array("zeroth", "first", "second",  "third",  "fourth",  "fith", "sixth",
    "seventh", "eighth", "ninth", "tenth", "eleventh", "twelfth")

  protected val AGGREGATIONS = Map(
    0 -> "NONE",
    1 -> "SUM",
    2 -> "AVERAGE",
    3 -> "COUNT")
  ...
  def parseText(text: String): Array[TapasNumericValueSpan] = {
    ...
    NUMBER_PATTERN.findAllMatchIn(text).foreach(m => {
            ...
            tableCellValues(pos._1)
              ...
              .foreach(tokenIndex =>
                numericRelations(tokenIndex) = relations
                  .map(rel => math.pow(2, rel - TapasNumericRelation.EQ)).sum.toInt)
        }
