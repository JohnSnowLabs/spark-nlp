...
class DistilBertForZeroShotClassification(override val uid: String)
  ...
  def setModelIfNotSet(
      ...
      openvinoWrapper: Option[OpenvinoWrapper]): DistilBertForZeroShotClassification = {
    if (_model.isEmpty) {
      _model = Some(
...
trait ReadDistilBertForZeroShotDLModel extends ReadTensorflowModel with ReadOnnxModel with ReadOpenvinoModel{
  ...
  def readModel(
    ...
    instance.getEngine match {
      ...
      case Openvino.name =>
        val openvinoWrapper = readOpenvinoModel(path, spark, "_distilbert_classification_openvino")
