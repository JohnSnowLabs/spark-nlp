...
/** XlmRoBertaForQuestionAnswering can load XLM-RoBERTa Models with a span classification head on
  ...
  * examples, see
  * [[https://github.com/JohnSnowLabs/spark-nlp/blob/master/src/test/scala/com/johnsnowlabs/nlp/annotators/classifier/dl/XlmRoBertaForQuestionAnsweringTestSpec.scala XlmRoBertaForQuestionAnsweringTestSpec]].
  *
  * ==Example==
  * {{{
  ...
  */
class XlmRoBertaForQuestionAnswering(override val uid: String)
    extends AnnotatorModel[XlmRoBertaForQuestionAnswering]
  ...
  /** Annotator reference id. Used to identify elements in metadata or to refer to this annotator
    * type
    */
  def this() = this(Identifiable.randomUID("XlmRoBertaForQuestionAnswering"))
  ...
  /** @group setParam */
  def setConfigProtoBytes(bytes: Array[Int]): XlmRoBertaForQuestionAnswering.this.type =
  ...
  def setModelIfNotSet(
      ...
      spp: SentencePieceWrapper): XlmRoBertaForQuestionAnswering = {
    if (_model.isEmpty) {
      _model = Some(
  ...
  override def onWrite(path: String, spark: SparkSession): Unit = {
    ...
    writeTensorflowModelV2(
      ...
      XlmRoBertaForQuestionAnswering.tfFile,
      configProtoBytes = getConfigProtoBytes)
    writeSentencePieceModel(
      path,
      spark,
      ...
      "_xlmroberta",
      XlmRoBertaForQuestionAnswering.sppFile)
  }
}

trait ReadablePretrainedXlmRoBertaForQAModel
    extends ParamsAndFeaturesReadable[XlmRoBertaForQuestionAnswering]
    with HasPretrained[XlmRoBertaForQuestionAnswering] {
  override val defaultModelName: Some[String] = Some("xlm_roberta_base_qa_squad2")

  /** Java compliant-overrides */
  override def pretrained(): XlmRoBertaForQuestionAnswering = super.pretrained()

  override def pretrained(name: String): XlmRoBertaForQuestionAnswering =
    super.pretrained(name)

  override def pretrained(name: String, lang: String): XlmRoBertaForQuestionAnswering =
    super.pretrained(name, lang)

  override def pretrained(
      name: String,
      lang: String,
      remoteLoc: String): XlmRoBertaForQuestionAnswering =
...
trait ReadXlmRoBertaForQATensorflowModel extends ReadTensorflowModel with ReadSentencePieceModel {
  ...
  def loadSavedModel(tfModelPath: String, spark: SparkSession): XlmRoBertaForQuestionAnswering = {
    ...
    new XlmRoBertaForQuestionAnswering()
      .setSignatures(_signatures)
      .setModelIfNotSet(spark, wrapper, spp)
  }
}

/** This is the companion object of [[XlmRoBertaForQuestionAnswering]]. Please refer to that class
