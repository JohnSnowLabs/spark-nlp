...
class PerceptronModel(override val uid: String) extends AnnotatorModel[PerceptronModel] with PerceptronUtils {
  ...
  def tag(tokenizedSentences: Array[TokenizedSentence]): Array[TaggedSentence] = {
    ...
    tokenizedSentences.map(sentence => {
      ...
      sentence.indexedTokens.zipWithIndex.map { case (IndexedToken(word, begin, end, sid), i) =>
...
trait PretrainedPerceptronModel {
  def pretrained(name: String = "pos_fast", language: Option[String] = Some("en"), remoteLoc: String = ResourceDownloader.publicLoc): PerceptronModel =
    ResourceDownloader.downloadModel(PerceptronModel, name, language, remoteLoc)
}
