    ...
    task match {
      case NER =>
        ...
        val rdd = withPred
          ...
          .map(r =>
            (
              r.getAs[scala.collection.Seq[String]](0).toSeq,
              r.getAs[scala.collection.Seq[String]](1).toSeq))
      ...
      case WordSegmentation =>
        ...
        val rdd = withPred
          ...
          .map { r =>
            val predBoundaries =
              r.getAs[scala.collection.Seq[scala.collection.Seq[Int]]](0)
                .map(b => (b.head, b(1)))
                .toSet
            val goldBoundaries = r.getAs[scala.collection.Seq[String]](1).map(parseIntPair).toSet
      ...
      case ImageClassification =>
        ...
        val rdd = predicted
          ...
          .rdd
          .map { r =>
            val annotations = r.getAs[scala.collection.Seq[Row]](0)
      ...
      case DependencyParsing =>
        ...
        val rdd = withPred
          ...
          .rdd
          .map { r =>
            val heads = r.getAs[scala.collection.Seq[Int]](0).toSeq
            val labels = r.getAs[scala.collection.Seq[String]](1).toSeq
            val pred = heads.zip(labels)
            val gold = r.getAs[scala.collection.Seq[String]](2).toSeq.map(parseHeadLabel)
            (pred, gold)
          }
      ...
      case QuestionAnswering =>
        ...
        val rdd = predicted
          ...
          .map { r =>
            val golds =
              if (multiRef) r.getAs[scala.collection.Seq[String]](1).toSeq
              else Option(r.getString(1)).toSeq
            (r.getString(0), golds)
          }
  ...
  private def labelAccuracy(
    ...
    if (!flatten) {
      ...
      val rdd = df
        ...
        .rdd
        .flatMap { r =>
          val pred = r.getAs[scala.collection.Seq[String]](0)
          val gold = r.getAs[scala.collection.Seq[String]](1)
          require(
