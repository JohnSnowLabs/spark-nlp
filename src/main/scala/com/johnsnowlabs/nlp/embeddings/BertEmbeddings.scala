...
class BertEmbeddings(override val uid: String)
  ...
  def batchAnnotate(batchedAnnotations: Seq[Array[Annotation]]): Seq[Seq[Annotation]] = {
    val tokenizedSentences = batchedAnnotations
      .flatMap(annotations => TokenizedWithSentence.unpack(annotations))
//      .filter(annotations => annotations.nonEmpty) // FixMe: There was an if statement cleaning up here
      .toArray
    ...
    withEmbeddings.map(embeddingsToPack => WordpieceEmbeddingsSentence.pack(embeddingsToPack))
  }
