...
class WordEmbeddingsModel(override val uid: String)
  ...
  val enableInMemory = new BooleanParam(
    this,
    "enableInMemory",
    "In-memory embeddings lookup. Faster but requires more memory")

  def setEnableInMemory(value: Boolean): this.type = set(enableInMemory, value)

  /** Dictionary of words with their vectors
    *
    * @group param
    */
  private[embeddings] val wordVectors: MapFeature[String, Array[Float]] =
    new MapFeature(this, "wordVectors")

  /** @group setParam */
  private[embeddings] def setWordVectors(value: Map[String, Array[Float]]): this.type =
    set(wordVectors, value)
  ...
  override def annotate(annotations: Seq[Annotation]): Seq[Annotation] = {
    ...
    val withEmbeddings = sentences.map { sentence =>
      val tokens = sentence.indexedTokens.map { token =>
        val (vectorOption, zeroArray) = retrieveEmbeddings(token.token)
      ...
      WordpieceEmbeddingsSentence(tokens, sentence.sentenceIndex)
    }
  ...
  private def retrieveEmbeddings(token: String): (Option[Array[Float]], Array[Float]) = {
    if (! $(enableInMemoryStorage)) {
      val vectorOption: Option[Array[Float]] = getReader(Database.EMBEDDINGS).lookup(token)
      val zeroVector: Array[Float] = getReader(Database.EMBEDDINGS).emptyValue
      (vectorOption, zeroVector)
    } else {
      val vectorOption: Option[Array[Float]] = $$(wordVectors).get(token)
      val zeroVector: Array[Float] = Array.fill($(dimension))(0.0f)
      (vectorOption, zeroVector)
    }
  }
