...
class CamemBertEmbeddings(override val uid: String)
  ...
  /** @group setParam */
  def setConfigProtoBytes(bytes: Array[Int]): CamemBertEmbeddings.this.type = set(this.configProtoBytes, bytes)
  ...
  def setMaxSentenceLength(value: Int): this.type = {
    require(value <= 512, "CamemBERT models do not support sequences longer than 512 because of trainable positional embeddings.")
  ...
  def setModelIfNotSet(spark: SparkSession, tensorflowWrapper: TensorflowWrapper): CamemBertEmbeddings = {
    if (_model.isEmpty) {
      _model = Some(
...
trait ReadablePretrainedBertModel
  ...
  override def pretrained(name: String, lang: String): CamemBertEmbeddings = super.pretrained(name, lang)
 ...
 * documentation.
 */
object CamemBertEmbeddings extends ReadablePretrainedBertModel with ReadBertTensorflowModel {
