...
class CoNLL2003NerReader(
  ...
  private val nerReader =
    CoNLL(documentCol = "document", sentenceCol = "sentence", tokenCol = "token", posCol = "pos")
      ...
      embeddingsFormat match {
        case ReadAs.TEXT =>
          WordEmbeddingsTextIndexer.index(
            wordEmbeddingsFile,
            new WordEmbeddingsWriter(connection, false, wordEmbeddingsNDims, 5000, 5000))
        case ReadAs.BINARY =>
          WordEmbeddingsBinaryIndexer.index(
            wordEmbeddingsFile,
            new WordEmbeddingsWriter(connection, false, wordEmbeddingsNDims, 5000, 5000))
      }
    }
  ...
  private val fg = FeatureGenerator(DictionaryFeatures.read(possibleExternalDictionary))
  ...
  private def readDataset(er: ExternalResource)
    ...
    labels
      .zip(posTaggedSentences.zip(withEmbeddings))
      .map { case (l, (p, w)) => (l, p, w) }
  }

  def readNerDataset(
      er: ExternalResource,
      metadata: Option[DatasetMetadata] = None): CrfDataset = {
    ...
    if (metadata.isEmpty)
      ...
      val labeledInstances = lines.map { line =>
        ...
        val labels =
          InstanceLabels(line._1.labels.map(l => metadata.get.label2Id.getOrElse(l, -1)))
        (labels, instance)
      }
